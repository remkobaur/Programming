...
class CL_DuplicatedFilesFinder:
    ...
    def get_FileList_for_folderTree(self,rootdir_glob,_config=config):
    ...
    def get_fileDict(self,fileName):
        return({
    ...
    def _addTags(self,item,search_list,_config):
    ...
    def findDuplicateList_Dict(self,item_list,_config = config):
        resultList =[]
        ...
        for item in item_list :        
            ...
            sameFileDictList = self._addTags(item,search_list,_config)
    ...
    def xlsExport_ValidatedDuplicateList(self,AnalysisResults,OutputFile):      
    ...
    def delete_DuplicatedFiles(self,AnalysisResults,deleteFiles):
        del_count = 0
        for sameFileDictList in AnalysisResults:
            for  k in sameFileDictList:
                if k['Delete'] == 'x':
    ...
    def process_folder(self,root = r'\\IR_MedServ\photo\Events\2014_09_27_Detmerode_Wald',_config = config):
        self.check_config(_config)   
        item_list = self.get_FileList_for_folderTree(rootdir_glob = root,_config=_config)    
        AnalysisResults=self.findDuplicateList_Dict(item_list,_config)
        fileName = root.replace('\\\\IR_MedServ\\photo\\','').replace('\\',' ')
        self.xlsExport_ValidatedDuplicateList(AnalysisResults,f'{fileName}.xlsx')
        self.delete_DuplicatedFiles(AnalysisResults,deleteFiles=_config['EnableDeleteFiles']) 
    
    def process_forEachSubfolder(self,root =r'\\IR_MedServ\photo\Events',_config = config):
        self.check_config(_config)           
        dirname = Path(root)
        ...
        for sf in subfolders:
            if sf in _config['skip_subFolders']:
                continue        
            item_list = self.get_FileList_for_folderTree(rootdir_glob = os.path.join(root,sf),_config=_config)
            AnalysisResults=self.findDuplicateList_Dict(item_list,_config)
            self.delete_DuplicatedFiles(AnalysisResults,deleteFiles=_config['EnableDeleteFiles']) 
            
    def process_forEachSubfolder_secondRootList(self,root =r'\\IR_MedServ\photo\Events', secondFolderList=[r'\\IR_MedServ\photo\share'],_config = config):
        ...
        for sf in subfolders:
            ...
            for secFolder in secondFolderList:
                item_list += self.get_FileList_for_folderTree(rootdir_glob = secFolder,_config=_config)
            AnalysisResults=self.findDuplicateList_Dict(item_list,_config)
            if len(AnalysisResults)>0:
                self.xlsExport_ValidatedDuplicateList(AnalysisResults,f'{fileName}.xlsx')
                delcount += self.delete_DuplicatedFiles(AnalysisResults,deleteFiles=_config['EnableDeleteFiles']) 
        print("=======================================") 
        print(f"Total deleted files = {delcount}") 
        
    def remove_Empty_subfolders(self,root,_config=config,topLevel = True):
        ...
        if isEmpty:
            ...
            status = True
            for sf in subfolders:                   
                status = self.remove_Empty_subfolders(os.path.join(root,sf),_config=_config,topLevel=False)
                if status ==False:
                    return False   
        return status 

    def process_doubleRoot_subfolderaofRoot1(self,root1 =r'\\IR_MedServ\photo\Events',root2= r'\\IR_MedServ\photo\Events', _config = config):
        ...
        for sf in subfolders1:
            ...
            if sf in subfolders2:
                ...
                item_list = self.get_FileList_for_folderTree(rootdir_glob = os.path.join(root1,sf),_config=_config)
                item_list += self.get_FileList_for_folderTree(rootdir_glob = os.path.join(root2,sf),_config=_config)    
                AnalysisResults=self.self.findDuplicateList_Dict(item_list,_config)
                if len(AnalysisResults)>0:
                    self.xlsExport_ValidatedDuplicateList(AnalysisResults,f'{sf}.xlsx')                
                    delcount += self.delete_DuplicatedFiles(AnalysisResults,deleteFiles=_config['EnableDeleteFiles'])
...
def testing1():
    ...
    item_list = DFF.get_FileList_for_folderTree(rootdir_glob = r'C:\GIT\_TestData',_config=_config)

    AnalysisResults=DFF.findDuplicateList_Dict(item_list,_config)
    DFF.xlsExport_ValidatedDuplicateList(AnalysisResults,'douplicate_List.xlsx')
    DFF.delete_DuplicatedFiles(AnalysisResults,deleteFiles=_config['EnableDeleteFiles']) 
    
def testing2():
    DFF = CL_DuplicatedFilesFinder()
    _config = copy.deepcopy(DFF.config)
    DFF.check_config(_config)   
    _config['remove_subFolders'] = ['Afterworld','Dias']
    ...
    item_list += DFF.get_FileList_for_folderTree(rootdir_glob = r'C:\GIT\_TestData\root1',_config=_config)
    item_list += DFF.get_FileList_for_folderTree(rootdir_glob = r'C:\GIT\_TestData\root02',_config=_config)
...
def testing3():
    ...
    item_list=[]
    item_list += DFF.get_FileList_for_folderTree(rootdir_glob = r'C:\GIT\_TestData1',_config=_config)
    item_list += DFF.get_FileList_for_folderTree(rootdir_glob = r'C:\GIT\_TestData2',_config=_config)
...
# _config['fileExtensions'] = ['.pdf','.xlsx','.docx','.pptx','.ppt']
# _config['keep_subFolders'] =[ r'\\ir_medserv\Backup\Remko\Docs\Promotion\Literatur' ] #'Best','best','uswahl',
...
# # process()
# # DFF.process_folder(r'\\IR_MedServ\photo\Events\2024_06_13_Promotion_Verteidigung',_config = _config)
# # DFF.process_folder(r'\\IR_MedServ\photo\Familie\Hzm_Mur(a)',_config = _config)
# # DFF.process_folder(r'\\IR_MedServ\Backup\Remko\_TestData',_config = _config)
# # DFF.process_folder(r'C:\GIT\_TestData',_config = _config)
# # DFF.process_forEachSubfolder(r'\\IR_MedServ\photo\DCIM-Handy\Remko',_config = _config)
# # DFF.process_forEachSubfolder_secondRootList(r'\\IR_MedServ\photo\Events',[r'\\IR_MedServ\photo\share'],_config = _config)
# # DFF.process_forEachSubfolder_secondRootList(r'\\IR_MedServ\photo\Familie\Baur_LuM\Urlaubsfotos',[],_config = _config)
# # DFF.remove_Empty_subfolders(root =r'\\IR_MedServ\photo\Familie\Hzm_Mur(a)\Fotos Petra',_config=_config)
# # DFF.process_doubleRoot_subfolderaofRoot1(root1 =r'\\IR_MedServ\photo\Familie\Hzm_Mur(a)\Fotos Petra',root2= r'\\IR_MedServ\photo\Urlaub', _config = _config)
# DFF.process_doubleRoot_subfolderaofRoot1(root1 =r'\\ir_medserv\Backup\Remko\Docs\Promotion\Literatur',root2= r'C:\Users\PC_Oxygen\Desktop\Backup2NAS\Literatur\To Citavi', _config = _config)
