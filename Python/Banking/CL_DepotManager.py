...
class CL_DepotManager:
    ...
    data_path = Path(r"E:\_NAS\0_Remko\Unterlagen\Banking\_Data")
    olb_path = data_path / "Parse_OLB"
    ...
    xls_db_path = path.join(data_path)
    ...
    def df_filter_isin_not(self,isin_list=[])  :
        ...
        self.df_history = self.df_history[~self.df_history["isin"].isin(isin_list)].reset_index(drop=True)
        self.df_change = self.df_change[~self.df_change["isin"].isin(isin_list)].reset_index(drop=True)
    def df_filter_start_date(self, start_date=None)  :
        if start_date is None:
            return
        self.df_history = self.df_history[self.df_history["date"] >= start_date].reset_index(drop=True)
        self.df_change = self.df_change[self.df_change["date"] >= start_date].reset_index(drop=True)
    def df_filter_end_date(self, end_date=None)  :
        if end_date is None:
            return
        self.df_history = self.df_history[self.df_history["date"] <= end_date].reset_index(drop=True)
        self.df_change = self.df_change[self.df_change["date"] <= end_date].reset_index(drop=True)
    #endregion
    
    def create_funds(self,reset_profit=False,start_date=None, end_date=None):
        ...
        for idx, isin in enumerate(unique_isins):
            ...
            fund.create_signals(start_date=start_date, end_date=end_date)
            fund.signals.calculate(reset_profit=reset_profit)
    ...
    def plot_all(self):
        ...
        fig, axs = plt.subplots(3, 3, figsize=(17, 9), sharex=True)
...
#region get current online price data
    ...
    def export_allsignals_to_xls(self, xls_file=None):
        if xls_file is None:
            xls_file = self.data_path.resolve() / "All_Fund_Signals.xlsx"
        try:
            os.makedirs(self.xls_db_path, exist_ok=True)
            with pd.ExcelWriter(xls_file, mode="w") as writer:
                for fund in self.funds:
                    df_signals = fund.signals.to_dataframe()
                    df_signals.to_excel(writer, sheet_name=f"{fund.isin}_signals", index=True)
            print(f"Exported all fund signals to: {xls_file}")
        except Exception as e:
            print(f"Could not export fund signals to Excel: {e}")
#endregion
...
if __name__ == "__main__":
    ...
    dm.xls_import_df_data(path.join(dm.data_path, "DepotManager_DB_Degussa_old.xlsx"))
